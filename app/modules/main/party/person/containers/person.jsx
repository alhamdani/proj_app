...
import ListView from '../../../../common/containers/table';
...
class Table extends React.Component{
  ...
  updateRows(){
    ...
    this.setState({ isDataLoaded : false });
    if( searchKey != '' ){
      console.log('trying to search for', searchKey)
      axios.post( QSearchUrl, { 'page' : page, 'limit' : limit, 'searchKey' : searchKey, 'columns' : columns})
        .then((rs)=>{
          let data = rs.data.qs;
          this.setUpVariables( data );
  ...
  contructTds(row){
    let { columns, mainUrl } = this.props;
    let { colSizes } = this.props;
    let editUrl = mainUrl+'moreinfo/'+row.id;
    return columns.map(( item, idx ) => {
      let size = colSizes[idx];
      return (
        <td key = { idx } className = {('w-'+size)}>
          <Link to = { editUrl } >
            { ( row[ item ] )}
          </Link>
        </td>
          ...
          <table className = 'tbl'>
            <thead>
              <tr>
                <th className = 'chxboxes'></th>
                <th className = 'nums'> # </th>
                <th className = 's1'>full name</th>
                <th className = 's3'>Position</th>
              </tr>
            </thead>
            <tbody>
              { rows }
